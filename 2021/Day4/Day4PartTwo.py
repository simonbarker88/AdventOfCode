...
def check_cards(number,boards):    
    ...
    for index,board in enumerate(boards):
        ...
        if is_bingo(board):
            ...
            if winners != None and index not in winners:
                winners.append(index)

            #print(board,index,number,len(winners),len(boards),winners)

            if len(winners) == len(boards):
                actual_card = index + 1
                print(f'The loser is board {actual_card}. The last number is {number}. Sum is {sum} and total is {total}')
                return True
